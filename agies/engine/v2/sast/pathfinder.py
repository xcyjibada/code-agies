...
class CallChainAnalyzer:
    ...
    def __init__(
        ...
        self._program_graph = program_graph  # Optional ProgramGraph
    ...
    def _build_graph(self) -> None:
        ...
        If a ``ProgramGraph`` is available (via constructor), uses its
        ``to_networkx()`` output for richer call-graph data (signals,
        scores).  Noise functions are excluded in both cases.
        """
        import networkx as nx

        G: Any = nx.DiGraph()

        # Prefer ProgramGraph when available — gives richer node attributes
        if self._program_graph is not None:
            nx_pg = self._program_graph.to_networkx()
            if nx_pg is not None:
                for u, v in nx_pg.edges():
                    if _is_noise_call(u) or _is_noise_call(v):
                        continue
                    G.add_edge(u, v)
                for node, data in nx_pg.nodes(data=True):
                    if not _is_noise_call(node):
