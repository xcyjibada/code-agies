...
from agies.engine.v2.feedback import FeedbackStore
from agies.engine.graph import GraphGenerator
...
def _detect_project_language(project_path: str) -> str | None:
    """Detect the dominant language of a project.

    Scans source files and returns ``"java"``, ``"js"``, ``"cpp"``,
    ``"python"``, or None if ambiguous/mixed.
    """
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        for fname in files:
            ...
            if ext in (".java", ".kt", ".scala"):
                lang_count["java"] = lang_count.get("java", 0) + 1
            elif ext in (".js", ".jsx", ".ts", ".tsx"):
                lang_count["js"] = lang_count.get("js", 0) + 1
            elif ext in (".c", ".h", ".cpp", ".hpp", ".cc", ".cxx"):
                lang_count["cpp"] = lang_count.get("cpp", 0) + 1
            elif ext in (".go",):
                lang_count["go"] = lang_count.get("go", 0) + 1
            elif ext in (".py", ".pyw"):
                lang_count["python"] = lang_count.get("python", 0) + 1
    ...
    return dominant if lang_count[dominant] > 0 else None


def _select_generator_for_language(
    lang: str | None,
) -> GraphGenerator | None:
    """Select the best graph generator for a project language.

    - Java/JS/TS/C++/Go → ``JoernGraphGenerator`` (Docker-based CPG)
    - Python → None (use existing tree-sitter path)
    - None/unknown → None (tree-sitter fallback)
    """
    if lang in ("java", "js", "cpp", "go"):
        logger.info(
            "Director: detected %s project — activating Joern graph generator",
            lang,
        )
        from agies.engine.graph.joern import JoernGraphGenerator
        if JoernGraphGenerator.check_docker_available():
            return JoernGraphGenerator()
        else:
            logger.warning(
                "Joern Docker image not available. "
                "Run: bash scripts/build_joern_docker.sh --proxy"
            )
            logger.info("Falling back to tree-sitter for %s project", lang)
    return None
...
class Director:
    ...
    def __init__(
        ...
        auto_select_generator: bool = True,
    ) -> None:
        ...
        self._graph_generator: GraphGenerator | None = graph_generator
        self._auto_select_generator = auto_select_generator
    ...
    def run(
        ...
        When ``graph_generator`` was provided to the constructor, delegates
        to the generator and converts ``ProgramSlice`` results back to
        ``EntryAnalysisCard`` for backward compatibility.
        ...
        """
        # ---------------------------------------------------------------
        # Auto-select generator for non-Python projects (Java/JS/C++/Go)
        # ---------------------------------------------------------------
        if self._graph_generator is None and self._auto_select_generator:
            lang = _detect_project_language(self.project_path)
            gen = _select_generator_for_language(lang)
            if gen is not None:
                self._graph_generator = gen

        # ---------------------------------------------------------------
        # Delegated path: use the pluggable GraphGenerator
        # ---------------------------------------------------------------
        if self._graph_generator is not None:
            return self._run_via_generator(max_cards, library_mode)
    ...
    def _run_via_generator(
        ...
        """Delegate to ``_graph_generator`` and convert results.

        The ``ProgramSlice`` → ``EntryAnalysisCard`` conversion ensures
        that all downstream consumers (Brain, agents) continue to work
        unchanged.
        """
        assert self._graph_generator is not None

        # Detect entry points (same heuristic as the existing path)
        if not self.entry_points:
            fnames = _walk_source_files(self.project_path)
            rm = self._repomap or RepoMap(root=self.project_path)
            _G, _pr, _rt, file_tags = rm.build_graph(
                fnames=fnames,
                entry_points=set(),
                signal_mul=self.signal_mul,
            )
        ...
        pg = self._graph_generator.build_program_graph(
        ...
        # Create slices from the ProgramGraph
        slices = self._graph_generator.create_slices(
            pg,
            entry_points=self.entry_points,
            signal_mul=self.signal_mul,
            max_slices=max_cards,
        )

        # Convert to EntryAnalysisCard
        cards = [sl.to_entry_analysis_card() for sl in slices]

        # Refine entry file_path/line_number
        for card in cards:
            for fn in card.functions_involved:
                if fn.name == card.entry:
                    card.file_path = fn.file_path
        ...
        self._last_cards = cards
        logger.info(
            "Director (via generator): produced %d cards (top: %s = %.4f)",
            len(cards),
            cards[0].entry if cards else "none",
            cards[0].final_score if cards else 0,
        )
